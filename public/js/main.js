$(document).ready(function(){
    ...
    BV.init();
    if (Modernizr.touch) {
        BV.show('assets/images/background1.jpg');
    } else {
        BV.show('assets/video/timelapse.mp4',
            {ambient:true,
            doLoop:true,
            altSource:'assets/video/timelapse.ogv'});
...
function getPricing(elem){
	$("#line1").text("");
	var value=$(elem).val();
	if(value=="United States"){
		$("#line1").text('(USD) $ 25 Per branch per month *');
	}
	if(value=="United Kingdom"){
		$("#line1").text('(GBP) £ 15 per branch per month');
	}
	if(value=="Europe"){
		$("#line1").append('EUR € 18 per branch per month');
	}
	if(value=="India"){
		$("#line1").text('(INR)1250 per branch per month');
	}
	if(value=="United Arab Emirates"){
		$("#line1").text('(AED) 90 Dirham per branch per month');
	}
}
