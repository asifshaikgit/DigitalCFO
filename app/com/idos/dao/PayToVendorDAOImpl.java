...
public class PayToVendorDAOImpl implements PayToVendorDAO {
	private static JPAApi jpaApi;
	private static EntityManager entityManager;

	@Inject
	public PayToVendorDAOImpl(JPAApi jpaApi) {
		this.jpaApi = jpaApi;
		entityManager = EntityManagerProvider.getEntityManager();
	}

	@Override
	public boolean submitForAprroval(Users user, JsonNode json, EntityManager entityManager,
		...
		try {
			...
			if (txnforproject != null && !txnforproject.equals("")) {
				txnProject = genericDao.getById(Project.class, Long.parseLong(txnforproject), entityManager);
			...
			if (!ConfigParams.getInstance().isDeploymentSingleUser(user)) {
				...
				TransactionViewResponse.addActionTxn(transaction.getId(), branchName, projectName, txnItem.getName(),
						itemParentName, "", "", "", "", vend.getName(),
						transaction.getTransactionPurpose().getTransactionPurpose(),
						idosdf.format(transaction.getTransactionDate()), invoiceDateLabel, invoiceDate, "", 0.0, 0.0,
						0.0, transaction.getNetAmount(), txnResultDesc, "", transaction.getTransactionStatus(),
						...
						transaction.getTransactionPurpose().getId(), "", "", 0, transaction.getTransactionRefNumber(),
						0l, 0.0, 0, typeOfSupply, result);
			}
			// SingleUser
			if (ConfigParams.getInstance().isDeploymentSingleUser(user)) {
