...
public class CodeHelper {
  //o_clusterNOK: Review: Done by cg (add factor 1000000000000), What happens in clustermode, same ID on different VM ?
	...
	private static AtomicLong ramid = new AtomicLong(1* 1000000000 );
	private static AtomicLong timeuniqueId = new AtomicLong(System.currentTimeMillis() * 64);
	...
	public static String getForeverUniqueID(String prefix,String postfix){
		String number="";
		if(prefix==null)
			prefix="";
		if(postfix==null)
			postfix="";
		number=prefix+getForeverUniqueID()+postfix;
		return number;
	}
