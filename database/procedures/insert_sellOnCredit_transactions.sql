DELIMITER $$
DROP PROCEDURE IF EXISTS insert_sellOnCredit_transactions;
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_sellOnCredit_transactions`(IN userId int(40),IN debugStoredPorc int(1))
BEGIN
-- from table
...
DECLARE temp_cursor CURSOR FOR 
 select * from temp_sell_on_credit_trans;
...
SELECT 'An error has occurred, operation rollbacked and the stored procedure was terminated' INTO @errorString;
 insert into  LOG_STORED_PROC(MESSAGE,NAME) values('Error SQLEXCEPTION',"insert sellOnCredit");
END;
  	


INSERT INTO sell_on_credit_seq(counter) VALUES (NULL);
SELECT 
    MAX(counter)
INTO v_bulkId FROM
    sell_on_credit_seq;
...
OPEN temp_cursor;
get_temp: LOOP
 FETCH temp_cursor INTO v_srNo,v_date,v_branch,v_project,v_typeOfSupply,v_withOrWithoutGST,v_cust,v_poReference,v_itemCode,v_price,v_units,v_discountPrcnt,v_advAdjstmnt,v_note;
 IF v_finished = 1 THEN 
 LEAVE get_temp;
 END IF;
 IF(debugStoredPorc=1) THEN
  insert into LOG_STORED_PROC(MESSAGE,NAME) values('Processing data for Sr. No',"insert sell on credit");
END IF;  
...
insert into  LOG_STORED_PROC(MESSAGE,NAME) values('Tax3=',v_tax3);
END IF;
IF(v_taxName LIKE 'CESS%') THEN
set v_branchTax_id4=branch_tax_id;
set v_tax4=v_netAmount*(v_taxRate/100.00);
...
TRANSACTION_STATUS,TRANSACTION_PURPOSE,CUSTOMER_DUE_PAYMENT,NET_AMOUNT,TAX_NAME_1,TAX_VALUE_1,TAX_NAME_2,TAX_VALUE_2,TAX_NAME_4,TAX_VALUE_4,TRANSACTION_SPECIFICS, TRANSACTION_SPECIFICS_PARTICULARS,TYPE_OF_SUPPLY,NO_OF_UNITS,PRICE_PER_UNIT,GROSS_AMOUNT, TRANSACTION_ACTIONDATE,PRESENT_STATUS,SOURCE_GSTIN,DESTINATION_GSTIN,INVOICE_NUMBER,RECEIPT_DETAILS_TYPE,PERFORMA_INVOICE,CREATED_BY, CREATED_AT,PO_REFERENCE,REMARKS) 
values (v_branchId,v_orgId,v_custId,CONCAT("BULK",v_bulkId,"SRNO",v_srNo),"NOT-PAID","Accounted",2,v_invoice_value,v_invoice_value,v_taxName1,v_tax1,v_taxName2,v_tax2,v_taxName4,v_tax4,v_spec,v_particular_id,1,v_no_units,v_priceperunit,v_gross_amt,v_tran_date,1,v_sourceGstCode,v_destGstCode,invoiceNo,0,0,userId, now(),v_poReference,v_note);
...
values(v_tran_id,2,v_spec,v_taxType1,v_particular_id,v_tax1,0,v_tran_date,v_branchTax_id1,v_branchId,v_orgId,1, userId, now());


insert into TRIALBALANCE_TAXES(TRANSACTION_ID,TRANSACTION_PURPOSE,TRANSACTION_SPECIFICS,TAX_TYPE,TRANSACTION_SPECIFICS_PARTICULARS,CREDIT_AMOUNT,DEBIT_AMOUNT,DATE,BRANCH_TAXESID,BRANCH_ID,BRANCH_ORGNIZATION_ID,PRESENT_STATUS,CREATED_BY,CREATED_AT)
values(v_tran_id,2,v_spec,v_taxType2,v_particular_id,v_tax2,0,v_tran_date,v_branchTax_id2,v_branchId,v_orgId,1, userId, now());
...
END IF;
END IF;
insert into TRANSACTION_ITEMS(BRANCH_ID,ORGANIZATION_ID,TRANSACTION_ID,TRANSACTION_SPECIFICS,TRANSACTION_SPECIFICS_PARTICULARS,NO_OF_UNITS,PRICE_PER_UNIT,GROSS_AMOUNT,ADJUSTMENT_FROM_ADVANCE,NET_AMOUNT,INVOICE_VALUE,DISCOUNT_PERCENT,TAX_DESCRIPTION,TAX1_ID,TAX2_ID,TAX3_ID,TOTAL_TAX,TAX_NAME_1,TAX_VALUE_1,TAX_RATE_1,TAX_NAME_2,TAX_VALUE_2,TAX_RATE_2,TAX_NAME_4,TAX_VALUE_4,TAX_RATE_4,PRESENT_STATUS,CREATED_BY, CREATED_AT)
values(v_branchId,v_orgId,v_tran_id,v_spec,v_particular_id,v_no_units,v_priceperunit,v_gross_amt,v_advAdj,v_invoice_value,v_invoice_value,v_discountPercent,v_taxFormula,v_branchTax_id1,v_branchTax_id2,v_branchTax_id4,v_tax,v_taxName1,v_tax1,v_taxRate1,v_taxName2,v_tax2,v_taxRate2,v_taxName4,v_tax4,v_taxRate4,1,userId, now());
...
values (v_branchId,v_orgId,v_custId,CONCAT("BULK",v_bulkId,"SRNO",v_srNo),"NOT-PAID","Accounted",2,v_invoice_value,v_invoice_value,v_taxName3,v_tax3,v_taxName4,v_tax4,v_spec,v_particular_id,1,v_no_units,v_priceperunit,v_gross_amt,v_tran_date,1,v_sourceGstCode,v_destGstCode,invoiceNo,0,0,userId, now(),v_poReference,v_note);
...
INTO v_tran_id FROM
 ...
 IF(v_invoice_value!=v_netAmount) then   
insert into TRIALBALANCE_TAXES(TRANSACTION_ID,TRANSACTION_PURPOSE,TRANSACTION_SPECIFICS,TAX_TYPE,TRANSACTION_SPECIFICS_PARTICULARS,CREDIT_AMOUNT,DEBIT_AMOUNT,DATE,BRANCH_TAXESID,BRANCH_ID,BRANCH_ORGNIZATION_ID,PRESENT_STATUS,CREATED_BY,CREATED_AT)
values(v_tran_id,2,v_spec,v_taxType3,v_particular_id,v_tax3,0,v_tran_date,v_branchTax_id3,v_branchId,v_orgId,1, userId, now());
...
insert into TRIALBALANCE_COAITEMS(TRANSACTION_ID,TRANSACTION_PURPOSE,TRANSACTION_SPECIFICS,TRANSACTION_SPECIFICS_PARTICULARS,CREDIT_AMOUNT,DEBIT_AMOUNT,DATE,BRANCH_ID,BRANCH_ORGNIZATION_ID,PRESENT_STATUS,CREATED_BY, CREATED_AT)
values(v_tran_id,2,v_spec,v_particular_id,v_gross_amt,0,v_tran_date,v_branchId,v_orgId,1, userId, now());
...
BRANCH_ORGNIZATION_ID,VENDOR_ID,VENDOR_TYPE,DEBIT_AMOUNT,CREDIT_AMOUNT,DATE,PRESENT_STATUS,CREATED_BY, CREATED_AT) values(v_tran_id,2,v_spec,v_particular_id,v_branchId,v_orgId,v_custId,2,v_invoice_value,0,v_tran_date, 1, userId, now());
...
DECLARE v_taxFormula3  VARCHAR(200) DEFAULT NULL;
DECLARE v_taxFormula1  VARCHAR(100) DEFAULT NULL;
DECLARE v_taxFormula2  VARCHAR(100) DEFAULT NULL;
DECLARE v_taxFormula4  VARCHAR(100) DEFAULT NULL;
...
set v_taxFormula3=CONCAT(v_taxName3,'(+',v_taxRate3,'%);',v_tax3);
set v_tax=v_tax3;
insert into  LOG_STORED_PROC(MESSAGE,NAME) values('TAX FORMULA3=',v_taxFormula3);
...
UPDATE TRANSACTION SET CUSTOMER_DUE_PAYMENT=v_totalValue WHERE ID=v_tran_id;
...
ELSE
insert into TRANSACTION_ITEMS(BRANCH_ID,ORGANIZATION_ID,TRANSACTION_ID,TRANSACTION_SPECIFICS,TRANSACTION_SPECIFICS_PARTICULARS,NO_OF_UNITS,PRICE_PER_UNIT,GROSS_AMOUNT,ADJUSTMENT_FROM_ADVANCE,NET_AMOUNT,INVOICE_VALUE,DISCOUNT_PERCENT,TAX_DESCRIPTION,TAX1_ID,TAX2_ID,TAX4_ID,TOTAL_TAX,TAX_NAME_1,TAX_VALUE_1,TAX_RATE_1,TAX_NAME_2,TAX_VALUE_2,TAX_RATE_2,TAX_NAME_4,TAX_VALUE_4,TAX_RATE_4,PRESENT_STATUS,CREATED_BY, CREATED_AT)
values(v_branchId,v_orgId,v_tran_id,v_spec,v_particular_id,v_no_units,v_priceperunit,v_gross_amt,v_advAdj,v_netAmount,v_netAmount,v_discountPercent,v_taxFormula,v_branchTax_id1,v_branchTax_id2,v_branchTax_id4,v_tax,v_taxName1,v_tax1,v_taxRate1,v_taxName2,v_tax2,v_taxRate2,v_taxName4,v_tax4,v_taxRate4,1,userId, now());
...
UPDATE TRIALBALANCE_VENDOR_CUSTOMER SET DEBIT_AMOUNT=v_totalValue  WHERE TRANSACTION_ID=v_tran_id;
END;
END IF;
END LOOP get_temp;
CLOSE temp_cursor;
