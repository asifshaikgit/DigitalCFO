...
DELIMITER $$
DROP PROCEDURE IF EXISTS insert_buyOnCredit_transactions;
CREATE PROCEDURE `insert_buyOnCredit_transactions`(IN userId int(40),IN debugStoredPorc int(1))
BEGIN
-- from table
...
DECLARE temp_cursor CURSOR FOR select * from temp_buy_on_credit_trans;
...
INSERT INTO buy_on_credit_seq(counter) VALUES (NULL);
SELECT 
    MAX(counter)
INTO v_bulkId FROM
    buy_on_credit_seq;
...
get_temp: LOOP
 FETCH temp_cursor INTO v_srNo,v_date,v_branch,v_project,v_typeOfSupply,v_vend,v_poReference,v_itemCode,v_price,v_units,v_gstRate,v_cessRate,v_advAdjstmnt,v_note,
  ...
  v_import_reference_date,v_Reference_number,v_port_code;
...
values (v_branchId,v_orgId,v_vendId,CONCAT("BULK",v_bulkId,"SRNO",v_srNo),"NOT-PAID","Accounted",4,v_invoice_value,v_invoice_value,v_taxName1,v_tax1,v_taxName2,v_tax2,v_taxName4,v_tax4,v_spec,v_particular_id,1,v_no_units,v_priceperunit,v_gross_amt,0,v_tran_date,1,v_sourceGstCode,v_destGstCode,invoiceNo,0,userId, now(),v_poReference,v_note);
...
values(v_tran_id,4,v_spec,v_taxType1,v_particular_id,v_tax1,0,v_tran_date,v_branchTax_id1,v_branchId,v_orgId,1, userId, now());


insert into TRIALBALANCE_TAXES(TRANSACTION_ID,TRANSACTION_PURPOSE,TRANSACTION_SPECIFICS,TAX_TYPE,TRANSACTION_SPECIFICS_PARTICULARS,DEBIT_AMOUNT,CREDIT_AMOUNT,DATE,BRANCH_TAXESID,BRANCH_ID,BRANCH_ORGNIZATION_ID,PRESENT_STATUS,CREATED_BY,CREATED_AT)
values(v_tran_id,4,v_spec,v_taxType2,v_particular_id,v_tax2,0,v_tran_date,v_branchTax_id2,v_branchId,v_orgId,1, userId, now());

IF(v_branchTax_id4!=0) THEN
insert into TRIALBALANCE_TAXES(TRANSACTION_ID,TRANSACTION_PURPOSE,TRANSACTION_SPECIFICS,TAX_TYPE,TRANSACTION_SPECIFICS_PARTICULARS,DEBIT_AMOUNT,CREDIT_AMOUNT,DATE,BRANCH_TAXESID,BRANCH_ID,BRANCH_ORGNIZATION_ID,PRESENT_STATUS,CREATED_BY,CREATED_AT)
values(v_tran_id,4,v_spec,v_taxType4,v_particular_id,v_tax4,0,v_tran_date,v_branchTax_id4,v_branchId,v_orgId,1, userId, now());
END IF;
...
values (v_branchId,v_orgId,v_vendId,CONCAT("BULK",v_bulkId,"SRNO",v_srNo),"NOT-PAID","Accounted",4,v_invoice_value,v_invoice_value,v_taxName3,v_tax3,v_taxName4,v_tax4,v_spec,v_particular_id,1,v_no_units,v_priceperunit,v_gross_amt,0,v_tran_date,1,v_sourceGstCode,v_destGstCode,invoiceNo,0,userId, now(),v_poReference,v_note);
...
IF(v_invoice_value!=v_netAmount) then    
insert into TRIALBALANCE_TAXES(TRANSACTION_ID,TRANSACTION_PURPOSE,TRANSACTION_SPECIFICS,TAX_TYPE,TRANSACTION_SPECIFICS_PARTICULARS,DEBIT_AMOUNT,CREDIT_AMOUNT,DATE,BRANCH_TAXESID,BRANCH_ID,BRANCH_ORGNIZATION_ID,PRESENT_STATUS,CREATED_BY,CREATED_AT)
values(v_tran_id,4,v_spec,v_taxType3,v_particular_id,v_tax3,0,v_tran_date,v_branchTax_id3,v_branchId,v_orgId,1, userId, now());
IF(v_branchTax_id4!=0) THEN
insert into TRIALBALANCE_TAXES(TRANSACTION_ID,TRANSACTION_PURPOSE,TRANSACTION_SPECIFICS,TAX_TYPE,TRANSACTION_SPECIFICS_PARTICULARS,DEBIT_AMOUNT,CREDIT_AMOUNT,DATE,BRANCH_TAXESID,BRANCH_ID,BRANCH_ORGNIZATION_ID,PRESENT_STATUS,CREATED_BY,CREATED_AT)
values(v_tran_id,4,v_spec,v_taxType4,v_particular_id,v_tax4,0,v_tran_date,v_branchTax_id4,v_branchId,v_orgId,1, userId, now());
END IF;
...
values(v_tran_id,4,v_spec,v_particular_id,v_gross_amt,0,v_tran_date,v_branchId,v_orgId,1, userId, now());
...
BRANCH_ORGNIZATION_ID,VENDOR_ID,VENDOR_TYPE,DEBIT_AMOUNT,CREDIT_AMOUNT,DATE,PRESENT_STATUS,CREATED_BY, CREATED_AT) values(v_tran_id,4,v_spec,v_particular_id,v_branchId,v_orgId,v_vendId,1,0,v_invoice_value,v_tran_date, 1, userId, now());
...
UPDATE TRANSACTION SET VENDOR_DUE_PAYMENT=v_totalValue WHERE ID=v_tran_id;
...
UPDATE TRIALBALANCE_VENDOR_CUSTOMER SET CREDIT_AMOUNT=v_totalValue  WHERE TRANSACTION_ID=v_tran_id;
END;
END IF;
END LOOP get_temp;
CLOSE temp_cursor;
